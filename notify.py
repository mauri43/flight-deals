...
def format_deal(deal: Deal) -> tuple[str, str]:
    ...
    body = (
        ...
        f"${deal.price} round trip (threshold: ${deal.threshold})\n\n"
        f"{deal.flights_url}"
    )
    return title, body
...
def send_summary(deals: list[Deal]) -> bool:
    ...
    for d in sorted(deals, key=lambda x: x.price):
        ...
        lines.append(f"{emoji} ${d.price} {d.origin}→{d.destination_name} ({d.depart_date})")

    body = "\n".join(lines[:15])  # Cap at 15 to avoid huge messages
    if len(deals) > 15:
